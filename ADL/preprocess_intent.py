...
def build_vocab(
    ...
    embeddings: List[List[float]] = [
        glove.get(token, [random() * 2 - 1 for _ in range(glove_dim)]) # if token exists in glove, return the embedding in glove,
        for token in vocab.tokens                                      # else return the vector is composed of random number
    ]
...
def main(args):
    ...
    intents = set() # label set
    words = Counter() # the frequency of each word
